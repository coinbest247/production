var AffiliateController = function () {
    ...
    function loadData(isPageChanged) {
        $.ajax({
            ...
            url: '/Admin/InvestPackage/GetAffiliateProfitAllPaging',
            dataType: 'json',
            beforeSend: function () {
                be.startLoading();
            },
            success: function (response) {
                ...
                $.each(response.Results, function (i, item) {
                    render += Mustache.render(template, {
                        InvestAmount: be.formatNumber(item.InvestAmountInUSDT,4),
                        InterestRate: be.formatNumber(item.InterestRate,2),
                        ProfitAmount: be.formatNumber(item.ProfitAmount,4),
